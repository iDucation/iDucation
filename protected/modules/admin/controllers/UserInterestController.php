	...
	public function actionIndex()
	{
		...
		if (!Yii::app()->getRequest()->getIsAjaxRequest()) {
			...
			foreach($data as $row)
			{
			    ...
			    $action = '<p><a href="'.Yii::app()->createUrl('admin/user/edit/i/'.@$row->user_interest_id).'"><i class="fa fa-pencil"></i></a> <a href="#myModal'.@$row->user_interest_id.'" data-toggle="modal"><i class="glyphicon glyphicon-trash"></i></a>
			    			<div id="myModal'.@$row->user_interest_id.'" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		                      ...
		                      <div class="modal-footer">
		                        ...
		                        <a href="'.Yii::app()->createUrl('admin/user/delete/i/'.@$row->user_interest_id).'"><button class="btn btn-danger">Delete</button></a>
		                      </div>
		                      </div>
		                      </div>
		                    </div></p>';
			    ...
			    $record[] = array(
						htmlentities(@$row->user_interest_id),
			    		htmlentities(@$row->user_id),						
						htmlentities(@$row->discuss_id),
						$action
					);		    
			}
