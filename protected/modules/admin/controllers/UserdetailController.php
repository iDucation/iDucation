...
class UserdetailController extends Controller
{
	...
	public function actionIndex()
	{
		...
		if (!Yii::app()->getRequest()->getIsAjaxRequest()) {
			...
			foreach($data as $row)
			{
			    ...
			    $action = '<p><a href="'.Yii::app()->createUrl('admin/user/edit/i/'.@$row->user_detail_id).'"><i class="fa fa-pencil"></i></a> <a href="#myModal'.@$row->user_detail_id.'" data-toggle="modal"><i class="glyphicon glyphicon-trash"></i></a>
			    			<div id="myModal'.@$row->user_detail_id.'" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		                      ...
		                      <div class="modal-footer">
		                        ...
		                        <a href="'.Yii::app()->createUrl('admin/user/delete/i/'.@$row->user_detail_id).'"><button class="btn btn-danger">Delete</button></a>
		                      </div>
		                      </div>
		                      </div>
		                    </div></p>';
			    ...
			    $record[] = array(
						htmlentities(@$row->user_detail_id),
			    		htmlentities(@$row->user_id),						
						htmlentities(@$row->mypicture),
						htmlentities(@$row->phone),
						htmlentities(@$row->address),
						htmlentities(@$row->religion),
						htmlentities(@$row->aboutme),
						$action
					);		    
			}
	...
	public function actionAdd(){
		...
		$this->layout = "admin";
		$model = new User_detailForm;
		if(isset($_POST['User_detailForm'])){
			$model->attributes = $_POST['User_detailForm'];
			if($model->validate()){
				$model->save();
				$this->redirect(Yii::app()->createUrl('admin/userdetail'));	
			}else{
				Yii::app()->user->setFlash('Error','Error when validating data !');
				$this->redirect($this->createUrl('/admin/userdetail/add'));
			}
		}
		$model->saveType = 'add'; 
		$this->render('add_user_detail',array('model'=>$model));
